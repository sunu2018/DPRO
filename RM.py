...
try:
	for app in listApp:
		try:
			try:
				with open("RM.txt", "r") as token:
					authToken = token.read()
					if not authToken:
						client = LINE()
						with open("RM.txt","w") as token:
					...
					if msg.contentType == 0:
						if cmd == "turn off":
							...
							if settings["detectUnsend"] == True:
								client.sendMessage(to, "➧ Detect unsend message aktif")
							else:
								settings["detectUnsend"] = True
								client.sendMessage(to, "➧ Detect unsend message aktif")
						elif cmd == "unsend off":
							if settings["detectUnsend"] == False:
								client.sendMessage(to, "➧ Detect unsend message nonaktif")
							else:
							...
							for contact in contacts:
								client.sendMessage(contact, "➧ Broadcasting : \n{}".format(str(txt)))
							...
							groups = client.getGroupIdsJoined()
							for group in groups:
								client.sendMessage(group, "➧ Broadcasting : \n   {}".format(str(txt)))
							...
							if len(cond) == 1:
								...
								client.sendMessage(to, result)
								client.sendMessage(to, "➧ Untuk melihat info video, silahkan ketik :  {}Yvideo {}|_(Nomor)".format(str(setKey), str(search)))
							elif len(cond) == 2:
								num = int(str(cond[1]))
								if num <= len(data):
							...
							if len(cond) == 1:
								...
								ret_ += "\n\n➧ Untuk memutar music, silahkan ketik :  {}Play {}|_(Nomor)".format(str(setKey), str(search))
								client.sendMessage(to, str(ret_))
							elif len(cond) == 2:
								num = int(cond[1])
							...
							with requests.session() as web:
								...
								if len(cond) == 1:
									...
									ret_ += "\n\n➧ Untuk melihat liriknya, silahkan ketik : {}Lyric {}|_(Nomor)".format(str(setKey), str(query))
									client.sendMessage(to, ret_)
								elif len(cond) == 2:
									num = int(cond[1])
									if num <= len(result):
